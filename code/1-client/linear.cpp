...
int main(int argc, char** argv) {
    ...
    Mat x_batch(B,D), y_batch(B,1), tau0_batch(B,1), tau1_batch(D, 1);
    ...
    PRF prf_client;
	COMM_client += send_keys(&prf_client, 1, &dt);
    dt.send(io_client);
    
	COMM_client += merge_mat_same(train_data, &prf_client, io_client, party);
	COMM_client += merge_mat_same(train_label, &prf_client, io_client, party);
	
	prf_client.cal(z0);
	prf_client.cal(z1);
	
	COMM_client += merge_mat_same(tau0, &prf_client, io_client, party);
	COMM_client += merge_mat_same(tau1, &prf_client, io_client, party);
    ...
    for (int it = 0, start = 0; it < IT; ++it, start+=B) {
            ...
            tau1_batch = (tau1.row(it)).transpose();
