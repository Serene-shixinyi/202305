...
double send_half_mat_same(Mat& x, PRF *prf, int o, My_Buffer* dt) {
	int r = x.rows(), c = x.cols();
...
double distribute_mat_same(Mat& x, PRF *prf_alice, PRF *prf_bob, My_Buffer* dt_alice, My_Buffer* dt_bob) {
	assert((x.rows() & 1) == 0);
	double rs = 0;
	rs += send_half_mat_same(x, prf_alice, 0, dt_bob);
	rs += send_half_mat_same(x, prf_bob, 1, dt_alice);
	return rs;
}
